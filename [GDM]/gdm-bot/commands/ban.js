        ...
        if (result.length > 0) {
            ...
            fivemexports.ghmattimysql.execute("SELECT * FROM `vrp_users` WHERE id = ?", [adminpermid], (result) => {
                if (result) {
                    ...
                    if (params[1] == "-1") {
                        let newval = fivemexports.vrp.vrpbot('banDiscord', [params[0], "perm", `${reason}`, `${adminname}[${adminpermid}] via Discord.`])
                    }
                    else {
                        let newval = fivemexports.vrp.vrpbot('banDiscord', [params[0], params[1], `${reason}`, `${adminname}[${adminpermid}] via Discord.`])
                    }
                        let embed = {
                            "title": "Banned User",
