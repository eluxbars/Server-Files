...
RMenu.Add('vRPClothing', 'main', RageUI.CreateMenu("", "~r~GDM Clothing Menu",1300, 100, "clothes", "clothes"))
...
RMenu.Add('vRPClothing', 'wardrobesub',  RageUI.CreateSubMenu(RMenu:Get("vRPClothing", "main")))
RMenu.Add('wardrobesub', 'clothingsub',  RageUI.CreateSubMenu(RMenu:Get("vRPClothing", "wardrobesub")))
...
RageUI.CreateWhile(1.0, true, function()
    ...
    if RageUI.Visible(RMenu:Get('vRPClothing', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Barbers", nil, {}, true, function(Hovered, Active, Selected) 
                if Selected then
                    TriggerEvent("GDM:openBarberShop")
                    RageUI.ActuallyCloseAll()
                end
            end)
            RageUI.Button("Wardrobe", nil, {}, true, function(Hovered, Active, Selected) end, RMenu:Get("vRPClothing", "wardrobesub"))
        end)
    end
    ...
    if RageUI.Visible(RMenu:Get('vRPClothing', 'wardrobesub')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            TriggerServerEvent("Clothing:LoadWardrobe")
            ...
            RageUI.Button("~g~Save Outfit", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                ...
                if (GetOnscreenKeyboardResult()) then
                    ...
                    if result then
                        TriggerServerEvent("Clothing:SaveOutfit", result)
                    end
                end
            end
    ...
    if RageUI.Visible(RMenu:Get('wardrobesub', 'clothingsub')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            end, RMenu:Get('vRPClothing', 'wardrobesub'))
    
            RageUI.Button("~r~Remove Outfit", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent("Clothing:RemoveOutfit", currentOutfit)
                end
            end, RMenu:Get('vRPClothing', 'wardrobesub'))
        end)
    end
