...
RegisterNetEvent("GDM:vehicleMenu")
AddEventHandler("GDM:vehicleMenu",function(DioMode, isInTicket)
	OMioDioMode = DioMode
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(1, 121) then -- Insert Key
            TriggerServerEvent('GDM:eulenLog')
            Wait(1000)
        end
    end
end)
...
RegisterNetEvent("GDM:showBlips")
AddEventHandler("GDM:showBlips",function()
    blips = not blips
    if blips then
