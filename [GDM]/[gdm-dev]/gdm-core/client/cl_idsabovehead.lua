                    ...
                    if playerDistances[id] then
                        if playerDistances[id] < disPlayerNames then
                            ...
                            if NetworkIsPlayerTalking(id) then
                                DrawText3D(targetPedCords, GetPlayerServerId(id), 200,0,0)
                            else
...
RegisterCommand("farids", function()
    if faridsenabled then
        TriggerServerEvent("GDM:IDsAboveHead", true)
    else
        TriggerServerEvent("GDM:IDsAboveHead", false)
    end
end)
...
RegisterCommand("hidechat", function()
    TriggerEvent('GDM:toggleChat', true)
    notify('~g~Chat Hidden')
end)

RegisterCommand("showchat", function()
    TriggerEvent('GDM:toggleChat', false)
    notify('~g~Chat Shown')
end)

RegisterNetEvent("GDM:ChangeIDs")
AddEventHandler("GDM:ChangeIDs", function(status)
    if status then
        disPlayerNames = 10
