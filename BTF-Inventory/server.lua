...
local Inventory = module("BTFCars", "cfg/cfg_inventory")
...
AddEventHandler('BTF:FetchPersonalInventory', function()
    local source = source
    if not InventorySpamTrack[source] then
        ...
        if data and data.inventory then
            ...
            TriggerClientEvent('BTF:FetchPersonalInventory', source, FormattedInventoryData, vRP.computeItemsWeight({data.inventory}), vRP.getInventoryMaxWeight({UserId}))
...
AddEventHandler('BTF:RefreshInventory', function(source)
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
    if data and data.inventory then
        local FormattedInventoryData = {}
...
RegisterNetEvent('BTF:GiveItem')
AddEventHandler('BTF:GiveItem', function(itemId, itemLoc)
    local source = source
...
RegisterNetEvent('BTF:TrashItem')
AddEventHandler('BTF:TrashItem', function(itemId, itemLoc)
    local source = source
    if not itemId then  vRPclient.notify(source, {'~r~You need to select an item, first!'}) return end
    if itemLoc == "Plr" then
...
RegisterNetEvent('BTF:FetchTrunkInventory')
AddEventHandler('BTF:FetchTrunkInventory', function(spawnCode)
    local source = source
    ...
    vRP.getSData({carformat, function(cdata)
        ...
        TriggerClientEvent('BTF:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({cdata}), maxVehKg)
    end})
end)

RegisterNetEvent('BTF:FetchHouseInventory')
AddEventHandler('BTF:FetchHouseInventory', function(nameHouse)
    local source = source
    houseName = nameHouse
    local user_id = vRP.getUserId({source})
...
AddEventHandler('BTF:LockPick', function()
    ...
    if vRP.tryGetInventoryItem({user_id, "lockpick", 1, true}) then
        TriggerClientEvent('BTF:whatIsThis', source)
    end  
end)

RegisterNetEvent('BTF:UseItem')
AddEventHandler('BTF:UseItem', function(itemId, itemLoc)
    local source = source
    if not itemId then    vRPclient.notify(source, {'~r~You need to select an item, first!'}) return end
    if itemLoc == "Plr" then
...
RegisterNetEvent('BTF:MoveItem')
AddEventHandler('BTF:MoveItem', function(inventoryType, itemId, inventoryInfo, Lootbag)
    local source = source
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
                ...
                vRP.getSData({carformat, function(cdata)
                    ...
                    if cdata[itemId] and cdata[itemId].amount >= 1 then
                        ...
                        if weightCalculation <= vRP.getInventoryMaxWeight({UserId}) then
                            ...
                            TriggerClientEvent('BTF:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({cdata}), maxVehKg)
                            TriggerEvent('BTF:RefreshInventory', source)
            ...
            if LootBagEntities[inventoryInfo] ~= nil then  
                if LootBagEntities[inventoryInfo].Items[itemId] then 
                    ...
                    if weightCalculation <= vRP.getInventoryMaxWeight({UserId}) then
                        ...
                        local maxVehKg = 200
                        TriggerClientEvent('BTF:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({LootBagEntities[inventoryInfo].Items}), maxVehKg)                
                        TriggerEvent('BTF:RefreshInventory', source)
                    else 
                        vRPclient.notify(source, {'~r~You do not have enough inventory space.'})
                    end
...
RegisterNetEvent('BTF:MoveItemX')
AddEventHandler('BTF:MoveItemX', function(inventoryType, itemId, inventoryInfo, Lootbag)
    local source = source
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
    if InventoryCoolDown[source] then vRPclient.notify(source, {'~r~Inventory Cooldown.'}) return end
...
RegisterNetEvent('BTF:MoveItemAll')
AddEventHandler('BTF:MoveItemAll', function(inventoryType, itemId, inventoryInfo, vehid)
    local source = source
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
    if not itemId then  vRPclient.notify(source, {'~r~You need to select an item, first!'}) return end
    ...
    vRPclient.isInComa(source, {}, function(in_coma) 
        if not in_coma then
            if LootBagEntities[netid] then
                ...
                local user_id = vRP.getUserId({source})
                if user_id ~= nil then
                    TriggerClientEvent("BTF:PlaySound", source, "zipper")
...
RegisterNetEvent('BTF:CloseLootbag')
AddEventHandler('BTF:CloseLootbag', function()
    local source = source
    for i,v in pairs(LootBagEntities) do 
...
function CloseInv(source)
    TriggerClientEvent('BTF:InventoryOpen', source, false, false)
end
...
function OpenInv(source, netid, LootBagItems)
    ...
    TriggerClientEvent('BTF:InventoryOpen', source, true, true)
    local FormattedInventoryData = {}
    ...
    if vRP.hasPermission({UserId, "pd.armory"}) then
        ...
        local maxVehKg = 200
        TriggerClientEvent('BTF:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({LootBagItems}), maxVehKg)
