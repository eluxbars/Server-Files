...
AddEventHandler('es_bank:robberycomplete', function(reward)
	robbing = false
	TriggerEvent('chatMessage', 'SYSTEM', {255, 0, 0}, "Robbery done, you received: £" .. reward)
	bank = ""
	secondsRemaining = 0
	incircle = false
end)

RegisterNetEvent('HVC:StorerobberyBlip')
AddEventHandler('HVC:StorerobberyBlip', function(index)
	CreateBlipRobbery(index)
end)

function CreateBlipRobbery(index)
	SetBlipSprite(blips[index], 52)
	SetBlipColour(blips[index], 1)
	SetBlipScale(blips[index], 0.7)
	SetBlipAsShortRange(blips[index], true)
	BeginTextCommandSetBlipName("STRING")
	AddTextComponentString("Robbed Market")
	EndTextCommandSetBlipName(blips[index])
	Citizen.Wait(600*1000)
	SetBlipSprite(blips[index], 52)
	SetBlipColour(blips[index], 2)
	SetBlipScale(blips[index], 0.7)
	SetBlipAsShortRange(blips[index], true)
	BeginTextCommandSetBlipName("STRING")
...
Citizen.CreateThread(function()
    for k, v in pairs(banks) do
        blips[v.index] = AddBlipForCoord(v.position.x, v.position.y, v.position.z)
		SetBlipSprite(blips[v.index], 52)
		SetBlipColour(blips[v.index], 3)
		SetBlipScale(blips[v.index], 0.7)
		SetBlipAsShortRange(blips[v.index], true)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("Market")
		EndTextCommandSetBlipName(blips[v.index])
    end
end)
...
Citizen.CreateThread(function()
	while true do
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 2)then
						...
						if(IsControlJustReleased(1, 51))then
							if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey("WEAPON_UNARMED") then
								bank_DisplayHelpText("You don't have a weapon!")
								--print("dont have a weapon")
							else
								--print("started robbery" ..k)
		...
		if robbing then
			...
			elseif (Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) > 11)then
