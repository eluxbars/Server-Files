...
AddEventHandler('openBoot', function()
    ...
    if VehInRadius and IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then 
        ...
        TriggerServerEvent('URP:FetchTrunkInventory', NVeh, NetworkGetNetworkIdFromEntity(nearestVeh))
    end
end)

RegisterNetEvent("URP:FetchInventoryCL")
AddEventHandler("URP:FetchInventoryCL",function()
    TriggerServerEvent('URP:FetchPersonalInventory')
end)
    ...
    if not tvRP.isInComa({}) then
        if not inventoryOpen then
            ...
            if IsLootBagOpening then
                ...
                TriggerServerEvent('URP:CloseLootbag')
...
RegisterNetEvent("URP:OpenHomeStorage")
AddEventHandler("URP:OpenHomeStorage", function(toggle , houseName)
    if toggle == true then
        TriggerServerEvent('URP:FetchPersonalInventory')
        inventoryOpen = true; 
        SetNuiFocus(true, true)
        ...
        TriggerServerEvent('URP:FetchHouseInventory', houseName)
    else
        inventoryOpen = false;
        SetNuiFocus(false, false)
        SetNuiFocusKeepInput(false)
...
RegisterNetEvent('URP:InventoryOpen')
AddEventHandler('URP:InventoryOpen', function(toggle, lootbag)
...
RegisterNetEvent('URP:ToggleNUIFocus')
AddEventHandler('URP:ToggleNUIFocus', function(value)
    --print('focus', value)
    SetNuiFocus(value, value)
    SetNuiFocusKeepInput(value)
end)

RegisterNetEvent('URP:SendSecondaryInventoryData')
AddEventHandler('URP:SendSecondaryInventoryData', function(InventoryData, CurrentKG, MaxKg)
...
RegisterNetEvent('URP:FetchPersonalInventory')
AddEventHandler('URP:FetchPersonalInventory', function(table, CurrentKG, MaxKG)
...
RegisterNUICallback('UseBtn', function(data, cb)
    TriggerServerEvent('URP:UseItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('TrashBtn', function(data, cb)
    TriggerServerEvent('URP:TrashItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('GiveBtn', function(data, cb)
    TriggerServerEvent('URP:GiveItem', data.itemId, data.invType)
    cb(true)
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)
...
RegisterNUICallback('MoveBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('URP:MoveItem', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('URP:MoveItem', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('URP:MoveItem', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveXBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('URP:MoveItemX', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('URP:MoveItemX', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('URP:MoveItemX', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveAllBtn', function(data, cb)
    if not IsLootBagOpening then
        ...
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('URP:MoveItemAll', data.invType, data.itemId, VehTypeA, NetworkGetNetworkIdFromEntity(nearestVeh2)) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('URP:MoveItemAll', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('URP:MoveItemAll', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNetEvent('URP:LockPick2')
AddEventHandler('URP:LockPick2', function()
    TriggerServerEvent('URP:LockPick')
end)

RegisterNetEvent('URP:whatIsThis')
AddEventHandler('URP:whatIsThis', function()
        ...
        hasDoneIt = false
               ...
               if chance == 2 then
                   ...
                   --
                       ...
                       VehTypeC = nearestVeh
                       inventoryType = 'CarBoot'
                       TriggerServerEvent('URP:FetchTrunkInventory', GetEntityArchetypeName(nearestVeh), NetworkGetNetworkIdFromEntity(nearestVeh))                   
