...
function Controller.InitLights(state)
	local disabledExtras = {}
	for _, pattern in pairs(state.pattern) do
		for _, stage in pairs(pattern.stages) do
			for _, extraIndex in pairs(stage.Extras) do
				if not disabledExtras[extraIndex] then
					SetVehicleExtra(state.entity, extraIndex, true)
					disabledExtras[extraIndex] = true
				end
			end
		end
	end
