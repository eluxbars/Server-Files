    ...
    RageUI.IsVisible1(RMenu:Get('MUSICMENU','main'),true,false,true,function()
        if not a.active then 
            ...
            RageUI.Button("Help","Assistance message",{},true,function(k,l,m)
                if m then 
                    TriggerEvent("URP:showNotification",{text="The Video ID is the ID at the end of the YouTube URL after =",height="200px",width="auto",colour="#FFF",background="#32CD32",pos="bottom-right",icon="success"},5000)
                end 
            end)
        end 
    end,function()
            ...
            for o,p in pairs(i)do 
                if i[o]~=nil then 
                    ...
                    RageUI.ButtonWithStyle("ID: "..p[3].." - Started: "..p[5].."","Name: "..p[4].." Distance Away: "..distance.." metres",{RightLabel="→→→"},true,function(k,l,m)
                        if m then 
                            TriggerServerEvent("URP:adminStopSong",p[1])
                            table.remove(i,o)
                        end 
                    end)
                end 
...
RegisterNetEvent("URP:playDjSong")
AddEventHandler("URP:playDjSong",function(s,coords,t,u)
...
RegisterNetEvent("URP:requestCurrentProgress")
AddEventHandler("URP:requestCurrentProgress",function(s,coords)
    if not g then 
...
RegisterNetEvent("URP:toggleDjMenu")
AddEventHandler("URP:toggleDjMenu",function()
    RageUI.Visible(RMenu:Get('MUSICMENU','main'),true)
    h=true 
end)
RegisterNetEvent("URP:toggleDjAdminMenu")
AddEventHandler("URP:toggleDjAdminMenu",function(x)
    RageUI.Visible(RMenu:Get('MUSICMENU','admin'),true)
    i=x 
end)
RegisterNetEvent("URP:finaliseSong")
AddEventHandler("URP:finaliseSong",function(y)
    if a.active then 
        TriggerServerEvent("URP:playDjSongServer",y,a.coords)
    end 
end)
RegisterNetEvent("URP:updateDjVolume")
AddEventHandler("URP:updateDjVolume",function(coords,z)
    if specificSongPlaying(coords)then 
        SendNUIMessage({type='djVolume',volume=z})
    end 
end)
RegisterNetEvent("URP:stopSong")
AddEventHandler("URP:stopSong",function(coords)
    local A=false
    for o,p in pairs(c)do 
        if p[1]==coords then 
            A=true
...
RegisterNetEvent("URP:skipDj")
AddEventHandler("URP:skipDj",function(coords,B)
    if specificSongPlaying(coords)then 
        if B then 
...
function skipDj(B)
    if songPlaying()then 
        TriggerServerEvent("URP:skipServer",a.coords,B)
    end 
end
function setupDj()
    a.active=true
...
function stopDj()
    if songPlaying()then 
        TriggerServerEvent("URP:stopSongServer",a.coords)
    end
    a.active=false
...
function updateVolume()
    if songPlaying()then 
        TriggerServerEvent("URP:updateVolumeServer",a.coords,b.volume)
    end 
end
...
function func_checkDjSongs(J)
    for o,p in pairs(c)do 
        if#(J.playerCoords-p[1])>45.0 then 
            ...
            if not c[o][3]then 
                g=true
                TriggerServerEvent('URP:requestCurrentProgressServer',o,p[1])
                c[o][3]=true 
            end 
        end 
    end 
...
RegisterNUICallback('returnProgress',function(K,L)
    if K.progress~=nil then 
        if K.progress~=0 then 
            TriggerServerEvent("URP:returnProgressServer",d,e,K.progress)
        end 
    end
    L('return')
end)
RegisterNetEvent("URP:returnProgress")
AddEventHandler("URP:returnProgress",function(s,coords,M,N)
    if g then 
        if specificSongPlaying(coords)then 
            c[s]=nil
            c[s]={coords,s,true}
