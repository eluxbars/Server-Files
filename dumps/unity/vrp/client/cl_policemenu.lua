RMenu.Add('URPPoliceMenu', 'main', RageUI.CreateMenu("","URP Police",1250,100, "banners", "police"))
RMenu.Add('URPPoliceMenu', 'objectmenu',  RageUI.CreateSubMenu(RMenu:Get("URPPoliceMenu", "main")))
RMenu:Get('URPPoliceMenu', 'main'):SetPosition(1300, 100)
RMenu:Get('URPPoliceMenu', 'objectmenu'):SetPosition(1300, 100)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('URPPoliceMenu', 'main')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                RageUI.Button("Object Menu" , nil, { RightLabel = '→→→'}, true, function(Hovered, Active, Selected) end, RMenu:Get('URPPoliceMenu', 'objectmenu'))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('URPPoliceMenu', 'objectmenu')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('URPPoliceMenu', 'speedzones')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                ...
                RageUI.Button("Delete Speedzone" , nil, {}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        TriggerServerEvent('URP:RemoveZone')
...
RegisterCommand('pd', function()
  if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
    TriggerServerEvent('URP:OpenPoliceMenu')
  end
end)
...
RegisterNetEvent("URP:PoliceMenuOpened")
AddEventHandler("URP:PoliceMenuOpened",function()
  RageUI.Visible(RMenu:Get('URPPoliceMenu', 'main'), not RageUI.Visible(RMenu:Get('URPPoliceMenu', 'main')))
end)
...
function createZone() 
  ...
  if speed == 0 then 
    ...
    return
      ...
      TriggerServerEvent('URP:ActivateZone', message, speed, radius, x, y, z)
end

RegisterNetEvent('URP:ZoneCreated')
AddEventHandler('URP:ZoneCreated', function(speed, radius, x, y, z)
...
RegisterNetEvent('URP:RemovedBlip')
AddEventHandler('URP:RemovedBlip', function()
...
RegisterNetEvent("URP:DragPlayer")
AddEventHandler('URP:DragPlayer', function(pl)
    other = pl
    drag = not drag
end)
