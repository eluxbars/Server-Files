  ...
  while true do 
    if IsDisabledControlJustPressed(0,38) then
      if playerCanRespawn and in_coma and secondsTilBleedout < 2 then
        TriggerServerEvent("URP:PoliceCheck")
        TriggerServerEvent("URP:RebelCheck")
        TriggerServerEvent("URP:VIPCheck")
        TriggerServerEvent("URP:getHouses")
        tvRP.respawnPlayer()
        DoScreenFadeIn(900)
        TriggerEvent("URP:StartRespawnCam")
        TriggerEvent("URP:OpenRespawnMenu")
      end
      Wait(1000)
    end
    Wait(0)
...
RegisterNetEvent("URP:10Seconds")
AddEventHandler("URP:10Seconds", function()
    DeathAnim = 10
    while DeathAnim <= 10 and DeathAnim >= 0 do
        Wait(1000)
        DeathAnim = DeathAnim - 1
...
RegisterNetEvent("URP:FixClient")
AddEventHandler("URP:FixClient", function()
...
function IsVK(Weapon)
 ...
 WeaponNames = {
    ...
    [tostring(GetHashKey("WEAPON_VIPVANDAL"))] = 'VIP Vanval',
