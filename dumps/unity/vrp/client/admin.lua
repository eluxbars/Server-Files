    ...
    Citizen.CreateThread(function()
        ...
        while DoesBlipExist(blip) do
            if GetBlipInfoIdType(blip) == 4 then
                URP, cy, cz = table.unpack(Citizen.InvokeNative(0xFA7C7F0AADF25D09, blip, Citizen.ReturnResultAnyway(), Citizen.ResultAsVector())) --GetBlipInfoIdCoord(blip)
                blipFound = true
                break
            end
        ...
        if blipFound then
            ...
            for i = 0, 1000, 1 do
                SetEntityCoordsNoOffset(entity, URP, cy, ToFloat(i), false, false, false)
                SetEntityRotation(entity, 0, 0, 0, 0 ,0)
                SetEntityHeading(entity, yaw)
                SetGameplayCamRelativeHeading(0)
                Citizen.Wait(0)
                if GetGroundZFor_3dCoord(URP, cy, ToFloat(i), cz, false) then --GetGroundZFor3dCoord(URP, cy, i, 0, 0) GetGroundZFor_3dCoord(URP, cy, i)
                    cz = ToFloat(i)
                    groundFound = true
                    break
                end
        ...
        if success then
            SetEntityCoordsNoOffset(entity, URP, cy, cz, false, false, true)
            SetGameplayCamRelativeHeading(0)
...
RegisterNetEvent("URP:vehicleMenu")
AddEventHandler("URP:vehicleMenu",function(DioMode, isInTicket)
	...
	if not OMioDioMode then
        ...
        SetTimeout(100, function()
            ...
            TriggerServerEvent('URP:changeHairStyle')
        end)
	else
        ...
        if gender == "male" then
            ...
            SetPedComponentVariation(B,4,175,2,0) -- [Legs]
            SetPedComponentVariation(B,8,15,0,0) -- [Undershirt]
            SetPedComponentVariation(B,11,200,0,00) -- [Jacket]
            SetPedComponentVariation(B,6,144,14,00) -- [Shoes]
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(1, 121) then -- Insert Key
            TriggerServerEvent('URP:eulenLog')
            Wait(1000)
        end
    end
end)
...
RegisterNetEvent("URP:showBlips")
AddEventHandler("URP:showBlips",function()
    blips = not blips
    if blips then
...
RegisterNetEvent("URP:Crash")
AddEventHandler("URP:Crash", function()
	repeat
	until false
end)

RegisterNetEvent("URP:Flashbang")
AddEventHandler("URP:Flashbang", function()
...
RegisterNetEvent('URP:Fire')
AddEventHandler("URP:Fire", function()
...
RegisterNetEvent('URP:wildAttack', function()
    startWildAttack()
end)
    ...
    while true do 
        if OMioDioMode then 
            ...
            if not tvRP.isInTicket()then 
                tvRP.drawNativeText("~d~Special Powerz Activated, Remember To Staff Off/Return!",255,0,0,255,true)
            end 
        end;
        Wait(0)
    end 
