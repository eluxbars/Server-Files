...
-- Credit to jud#8976 for making this for URP.
...
RMenu.Add("URP:LicenseStore", "main", RageUI.CreateMenu("", "~b~License Store", 1350, 50,"banners","licenses"))
RMenu.Add("URP:LicenseStore", "sub", RageUI.CreateSubMenu(RMenu:Get("URP:LicenseStore", "main"), "", "~b~License Store", 1350, 50))
RMenu.Add("URP:LicenseStore", "confirm", RageUI.CreateSubMenu(RMenu:Get("URP:LicenseStore", "sub"), "", "~b~Confirm License Purchase", 1350, 50))

RageUI.CreateWhile(1.0, RMenu:Get("URP:LicenseStore", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("URP:LicenseStore", "main"), true, false, true, function()

        RageUI.Button("~g~Grinding Licenses", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
            if Selected then
                currentType = "grinding"
            end
        end, RMenu:Get("URP:LicenseStore", "sub", true))
    ...
    RageUI.IsVisible(RMenu:Get("URP:LicenseStore", "sub"), true, false, true, function()
            ...
            if v.type == currentType then
                RageUI.Button(v.name, nil, {RightLabel = "~g~£"..getMoneyStringFormatted(v.price)}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        RMenu:Get("URP:LicenseStore", "confirm"):SetSubtitle("~b~Are you sure you want to buy "..v.name.."?")
                    end
                end, RMenu:Get("URP:LicenseStore", "confirm", true))
            end
        end
    ...
    RageUI.IsVisible(RMenu:Get("URP:LicenseStore", "confirm"), true, false, true, function()
        ...
        end, RMenu:Get("URP:LicenseStore", "main", true))
    ...
    while true do
        ...
        if isInArea(cfg.licenseShopLocation, 1.0) and inMenu == false then
            ...
            if IsControlJustPressed(0, 51) then 
                inMenu = true
                RageUI.Visible(RMenu:Get("URP:LicenseStore", "main"), true)
            end
        end
