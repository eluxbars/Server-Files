...
URPClampClientT = {}
Tunnel.bindInterface("URP_Clamp",URPClampClientT)
Proxy.addInterface("URP_Clamp",URPClampClientT)
URPClampServer = Tunnel.getInterface("URP_Clamp","URP_Clamp")
vRP = Proxy.getInterface("vRP")

Clamps = {}
DisabledVehs = {}

function URPClampClientT.ClampVehicle()
  ...
  if DoesEntityExist(veh) then
    ...
    URPClampServer.ChangeVehState({VehToNet(veh), true})
...
function URPClampClientT.ChangeVehState(veh, disable)
  if disable then
...
RegisterCommand("removeclamps", function(source, args, rawCommand)
  for k, v in pairs(Clamps) do
    if v ~= nil then
      DeleteEntity(v[1])
      URPClampServer.ChangeVehState({VehToNet(v[2]), false})
    end
  end
  Clamps = {}
...
RegisterNetEvent("URP:UnClampVehicles")
AddEventHandler("URP:UnClampVehicles", function()
  for k, v in pairs(Clamps) do
    if v ~= nil then
      DeleteEntity(v[1])
      URPClampServer.ChangeVehState({VehToNet(v[2]), false})
...
RegisterCommand("removeclamp", function(source, args, rawCommand)
  ...
  if Clamps[ID] == nil then
    ...
    URPClampServer.ChangeVehState({VehToNet(Clamps[ID][2]), false})
    Clamps[ID] = nil
