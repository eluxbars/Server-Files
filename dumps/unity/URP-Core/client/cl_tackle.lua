...
RegisterNetEvent('URP:isPoliceVar')
AddEventHandler('URP:isPoliceVar', function(target)

end)

RegisterNetEvent('URP:getTackled')
AddEventHandler('URP:getTackled', function(target)
...
RegisterNetEvent('URP:playTackle')
AddEventHandler('URP:playTackle', function()
	isTackling = true
	local playerPed =PlayerPedId()
	...
	while true do
		...
		if IsControlPressed(0, Keys['LEFTSHIFT'], true) and IsControlPressed(0, Keys['G'], true) and not isTackling then
			...
			if distance ~= -1 and distance <= 3.0 and not isTackling and not isGettingTackled and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				TriggerServerEvent('URP:tryTackle', closestPlayerId)
			end
		end
	end
end)
