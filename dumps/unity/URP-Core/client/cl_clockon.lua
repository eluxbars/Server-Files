...
RMenu.Add('PDClockOn', 'main', RageUI.CreateMenu("", "~b~URP PD Clockon", 1300, 50, "banners", "police"))
    ...
    RageUI.IsVisible(RMenu:Get('PDClockOn', 'main'), true, false, true, function()
        for i , p in pairs(clockon) do 
            RageUI.Button(p.group, nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent('URP:Clockon', p.group)
                    TriggerEvent('URP:policeRemove')
                    TriggerServerEvent("URP:PoliceCheckRadial")
                end
            end)
        end
...
RegisterNetEvent('URP:policeRemove')
AddEventHandler('URP:policeRemove', function()
    local source = source
