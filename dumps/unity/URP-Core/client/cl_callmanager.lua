...
RMenu.Add('callmanager', 'main', RageUI.CreateMenu("", '~r~URP Call Manager', 1300, 50, "banners", "callmanager"))
                    ...
                    if k == ticketID then
                        if v[2] == GetPlayerServerId(PlayerId()) then
                            ...
                            CallManagerServer.GetUpdatedCoords({v[2]}, function(targetCoords)
                                ...
                                TriggerServerEvent("URP:GiveTicketMoney", v[1], v[2], v[3], true)
                            end)  
                            CallManagerServer.RemoveTicket({k, "admin"})
                            name = v[1]
                            isInTicket = true
                            TriggerServerEvent('URP:getTempFromPerm',v[2])
                        end
                    end
                end
            end
...
AddEventHandler("staffon", function(isInTicket)
    ...
    TriggerEvent("URP:vehicleMenu", true, isInTicket)
    if GetEntityHealth(GetPlayerPed(-1)) <= 103 then
    TriggerEvent('URP:FixClient')
    end
end)
...
AddEventHandler("staffoff", function()
    isInTicket = false
    TriggerEvent("URP:vehicleMenu", false, isInTicket) 
end)


RegisterNetEvent('URP:AdminTicketCooldown')
AddEventHandler('URP:AdminTicketCooldown', function(source, Reason)
    if AdminTicketCooldown == false then
        TriggerServerEvent('URP:sendAdminTicket', source, Reason)
...
RegisterNetEvent('URP:PDCallCooldown')
AddEventHandler('URP:PDCallCooldown', function(source, Reason)
    if PDCallCooldown == false then
        TriggerServerEvent('URP:sendPDCall', source, Reason)
        PDCallCooldown = true
        Wait(60000*2)
...
RegisterNetEvent('URP:NHSCallCooldown')
AddEventHandler('URP:NHSCallCooldown', function(source, Reason)
    if NHSCallCooldown == false then
        TriggerServerEvent('URP:sendNHSCall', source, Reason)
...
RegisterNetEvent('URP:PLAYTICKETRECIEVED')
AddEventHandler('URP:PLAYTICKETRECIEVED', function(source)
    PlaySoundFrontend(-1, "Apt_Style_Purchase", "DLC_APT_Apartment_SoundSet", 0)
end)

RegisterNetEvent('URP:sendPermID')
AddEventHandler('URP:sendPermID', function(permid)
    permid = permid
    while isInTicket do
        inRedzone = false
        Wait(0)
