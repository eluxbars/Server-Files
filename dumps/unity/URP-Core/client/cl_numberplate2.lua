...
local location = vector3(425.05426025391,1.2815328836441,91.935409545898)
local m = module("URPCars", "cfg/cfg_garages")
m=m.garage_types
...
RageUI.CreateWhile(1.0, RMenu:Get("PlateShop", "main"), nil, function()
    ...
    RageUI.IsVisible(RMenu:Get("PlateShop", "sub"), true, false, true, function()
        RageUI.Button("Change Number Plate", "~g~Changing plate of "..selectedCarName, {RightLabel = "~g~£50,000"}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("URP:ChangeNumberPlate", selectedCar)
            end
        end)
    ...
    while true do
        ...
        if isInArea(location, 1.4) and isInMenu == false then 
            ...
            if IsControlJustPressed(0, 51) then 
                TriggerServerEvent('URP:getCars')
...
RegisterNetEvent("URP:RecieveNumberPlate")
AddEventHandler("URP:RecieveNumberPlate", function(numplate)
...
RegisterNetEvent("URP:carsTable")
AddEventHandler("URP:carsTable",function(cars)
    carstable = cars
end)
