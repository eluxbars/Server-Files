...
RegisterCommand("farids", function()
    if faridsenabled then
        TriggerServerEvent("URP:IDsAboveHead", true)
    else
        TriggerServerEvent("URP:IDsAboveHead", false)
    end
end)
...
RegisterCommand("hidechat", function()
    TriggerEvent('URP:toggleChat', true)
    notify('~g~Chat Hidden')
end)

RegisterCommand("showchat", function()
    TriggerEvent('URP:toggleChat', false)
    notify('~g~Chat Shown')
end)

RegisterNetEvent("URP:ChangeIDs")
AddEventHandler("URP:ChangeIDs", function(status)
    if status then
        disPlayerNames = 10
