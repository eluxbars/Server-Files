...
Citizen.CreateThread(function()
    ...
    JayMenu.SetSubTitle('tattoo', "Tattoo Shop")
						...
						if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
							if tattoo.HashNameMale ~= '' then
								...
								if found then
									...
									local clicked, hovered = JayMenu.Button(GetLabelText(tattoo.Name), "~HUD_COLOUR_BLUE~→→→")
									if clicked then
										...
										TriggerServerEvent("SincoRP:saveTattoos", TattooData)
									elseif hovered then
...
RegisterNetEvent("SincoRP:setTattos")
AddEventHandler("SincoRP:setTattos",function(setTattos)
    if setTattos then
