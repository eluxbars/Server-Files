...
AddEventHandler('openBoot', function()
    ...
    if VehInRadius and IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then 
        ...
        TriggerServerEvent('SincoRP:FetchTrunkInventory', NVeh, NetworkGetNetworkIdFromEntity(nearestVeh))
    end
end)

RegisterNetEvent("SincoRP:FetchInventoryCL")
AddEventHandler("SincoRP:FetchInventoryCL",function()
    TriggerServerEvent('SincoRP:FetchPersonalInventory')
end)
    ...
    if not tvRP.isInComa({}) then
        if not inventoryOpen then
            ...
            if IsLootBagOpening then
                ...
                TriggerServerEvent('SincoRP:CloseLootbag')
...
RegisterNetEvent("SincoRP:OpenHomeStorage")
AddEventHandler("SincoRP:OpenHomeStorage", function(toggle , houseName)
    if toggle == true then
        TriggerServerEvent('SincoRP:FetchPersonalInventory')
        inventoryOpen = true; 
        SetNuiFocus(true, true)
        ...
        TriggerServerEvent('SincoRP:FetchHouseInventory', houseName)
    else
        inventoryOpen = false;
        SetNuiFocus(false, false)
        SetNuiFocusKeepInput(false)
...
RegisterNetEvent('SincoRP:InventoryOpen')
AddEventHandler('SincoRP:InventoryOpen', function(toggle, lootbag)
...
RegisterNetEvent('SincoRP:ToggleNUIFocus')
AddEventHandler('SincoRP:ToggleNUIFocus', function(value)
    --print('focus', value)
    SetNuiFocus(value, value)
    SetNuiFocusKeepInput(value)
end)

RegisterNetEvent('SincoRP:SendSecondaryInventoryData')
AddEventHandler('SincoRP:SendSecondaryInventoryData', function(InventoryData, CurrentKG, MaxKg)
...
RegisterNetEvent('SincoRP:FetchPersonalInventory')
AddEventHandler('SincoRP:FetchPersonalInventory', function(table, CurrentKG, MaxKG)
...
RegisterNUICallback('UseBtn', function(data, cb)
    TriggerServerEvent('SincoRP:UseItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('TrashBtn', function(data, cb)
    TriggerServerEvent('SincoRP:TrashItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('GiveBtn', function(data, cb)
    TriggerServerEvent('SincoRP:GiveItem', data.itemId, data.invType)
    cb(true)
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)
...
RegisterNUICallback('MoveBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('SincoRP:MoveItem', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('SincoRP:MoveItem', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('SincoRP:MoveItem', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveXBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('SincoRP:MoveItemX', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('SincoRP:MoveItemX', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('SincoRP:MoveItemX', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveAllBtn', function(data, cb)
    if not IsLootBagOpening then
        ...
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('SincoRP:MoveItemAll', data.invType, data.itemId, VehTypeA, NetworkGetNetworkIdFromEntity(nearestVeh2)) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('SincoRP:MoveItemAll', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('SincoRP:MoveItemAll', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNetEvent('SincoRP:LockPick2')
AddEventHandler('SincoRP:LockPick2', function()
    TriggerServerEvent('SincoRP:LockPick')
end)

RegisterNetEvent('SincoRP:whatIsThis')
AddEventHandler('SincoRP:whatIsThis', function()
        ...
        hasDoneIt = false
               ...
               if chance == 2 then
                   ...
                   --
                       ...
                       VehTypeC = nearestVeh
                       inventoryType = 'CarBoot'
                       TriggerServerEvent('SincoRP:FetchTrunkInventory', GetEntityArchetypeName(nearestVeh), NetworkGetNetworkIdFromEntity(nearestVeh))                   
