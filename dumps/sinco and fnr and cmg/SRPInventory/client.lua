...
AddEventHandler('openBoot', function()
    ...
    if VehInRadius and IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then 
        ...
        TriggerServerEvent('SRP:FetchTrunkInventory', NVeh, NetworkGetNetworkIdFromEntity(nearestVeh))
    else
...
RegisterCommand('inventory', function()
    if not tvRP.isInComa({}) then
        if not inventoryOpen then
            TriggerServerEvent('SRP:FetchPersonalInventory')
            inventoryOpen = true; 
            ...
            if IsLootBagOpening then
                ...
                TriggerServerEvent('SRP:CloseLootbag')
...
RegisterNetEvent("SRP:OpenHomeStorage")
AddEventHandler("SRP:OpenHomeStorage", function(toggle , houseName)
    if toggle == true then
        TriggerServerEvent('SRP:FetchPersonalInventory')
        inventoryOpen = true; 
        SetNuiFocus(true, true)
        ...
        TriggerServerEvent('SRP:FetchHouseInventory', houseName)
    else
        inventoryOpen = false;
        SetNuiFocus(false, false)
        SetNuiFocusKeepInput(false)
...
RegisterNetEvent('SRP:InventoryOpen')
AddEventHandler('SRP:InventoryOpen', function(toggle, lootbag)
...
RegisterNetEvent('SRP:ToggleNUIFocus')
AddEventHandler('SRP:ToggleNUIFocus', function(value)
    --print('focus', value)
    SetNuiFocus(value, value)
    SetNuiFocusKeepInput(value)
end)

RegisterNetEvent('SRP:SendSecondaryInventoryData')
AddEventHandler('SRP:SendSecondaryInventoryData', function(InventoryData, CurrentKG, MaxKg)
...
RegisterNetEvent('SRP:FetchPersonalInventory')
AddEventHandler('SRP:FetchPersonalInventory', function(table, CurrentKG, MaxKG)
...
RegisterNUICallback('UseBtn', function(data, cb)
    TriggerServerEvent('SRP:UseItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('TrashBtn', function(data, cb)
    TriggerServerEvent('SRP:TrashItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('GiveBtn', function(data, cb)
    TriggerServerEvent('SRP:GiveItem', data.itemId, data.invType)
    cb(true)
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)
...
RegisterNUICallback('MoveBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('SRP:MoveItem', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('SRP:MoveItem', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('SRP:MoveItem', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveXBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('SRP:MoveItemX', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('SRP:MoveItemX', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('SRP:MoveItemX', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveAllBtn', function(data, cb)
    if not IsLootBagOpening then
        ...
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('SRP:MoveItemAll', data.invType, data.itemId, VehTypeA, NetworkGetNetworkIdFromEntity(nearestVeh2)) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('SRP:MoveItemAll', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('SRP:MoveItemAll', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNetEvent('SRP:LockPick2')
AddEventHandler('SRP:LockPick2', function()
    TriggerServerEvent('SRP:LockPick')
end)

RegisterNetEvent('SRP:whatIsThis')
AddEventHandler('SRP:whatIsThis', function()
        ...
        hasDoneIt = false
               ...
               if chance == 3 then
                   ...
                   --
                       ...
                       VehTypeC = nearestVeh
                       inventoryType = 'CarBoot'
                       TriggerServerEvent('SRP:FetchTrunkInventory', GetEntityArchetypeName(nearestVeh), NetworkGetNetworkIdFromEntity(nearestVeh))                   
