...
SincoRPClampClientT = {}
Tunnel.bindInterface("SincoRP_Clamp",SincoRPClampClientT)
Proxy.addInterface("SincoRP_Clamp",SincoRPClampClientT)
SincoRPClampServer = Tunnel.getInterface("SincoRP_Clamp","SincoRP_Clamp")
vRP = Proxy.getInterface("vRP")

Clamps = {}
DisabledVehs = {}

function SincoRPClampClientT.ClampVehicle()
  ...
  if DoesEntityExist(veh) then
    ...
    DecorSetInt(clamp,"SincoRPCARS",955)
    ...
    SincoRPClampServer.ChangeVehState({VehToNet(veh), true})
...
function SincoRPClampClientT.ChangeVehState(veh, disable)
  if disable then
...
RegisterCommand("removeclamps", function(source, args, rawCommand)
  for k, v in pairs(Clamps) do
    if v ~= nil then
      DeleteEntity(v[1])
      SincoRPClampServer.ChangeVehState({VehToNet(v[2]), false})
    end
  end
  Clamps = {}
...
RegisterNetEvent("SincoRP:UnClampVehicles")
AddEventHandler("SincoRP:UnClampVehicles", function()
  for k, v in pairs(Clamps) do
    if v ~= nil then
      DeleteEntity(v[1])
      SincoRPClampServer.ChangeVehState({VehToNet(v[2]), false})
...
RegisterCommand("removeclamp", function(source, args, rawCommand)
  ...
  if Clamps[ID] == nil then
    ...
    SincoRPClampServer.ChangeVehState({VehToNet(Clamps[ID][2]), false})
    Clamps[ID] = nil
