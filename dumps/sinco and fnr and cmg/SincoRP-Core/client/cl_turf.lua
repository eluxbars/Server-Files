...
RegisterNetEvent('SincoRP:TakenTurf')
AddEventHandler('SincoRP:TakenTurf', function(isnTurf)
...
RegisterNetEvent('SincoRP:OutOfZone')
AddEventHandler('SincoRP:OutOfZone', function(isnTurf)
	inTurf = false

	-- TriggerEvent('chatMessage', '[SincoRP Turf System]:', {255, 34, 0}, "The turf cap was cancelled, you will receive nothing.")
...
RegisterNetEvent('SincoRP:PlayerDied')
AddEventHandler('SincoRP:PlayerDied', function(isnTurf)
	inTurf = false
	-- TriggerEvent('chatMessage', '[SincoRP Turf System]:', {255, 34, 0}, "The turf cap was cancelled, you died!")
...
RegisterNetEvent('SincoRP:TurfComplete')
AddEventHandler('SincoRP:TurfComplete', function(reward, name)
	inTurf = false
	-- TriggerEvent('chatMessage', '[SincoRP Turf System]:', {255, 34, 0}, "Turf capture sucessful, you received:^2 £" .. reward)
...
Citizen.CreateThread(function()
	while true do
				...
				if not inTurf then
					...
					if (Vdist(pos.x, pos.y, pos.z, pos3.x, pos3.y, pos3.z) < 1.4) then
						...
						if (IsControlJustReleased(1, 51)) then
							TriggerServerEvent('SincoRP:rob', k)
		...
		if inTurf then		
			...
			TriggerServerEvent('SincoRP:PlayerDied', turf)

			elseif (Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) > turfs[turf].radius) then
				TriggerServerEvent('SincoRP:TooFar', turf)
			end
		end
