...
local m = module("SincoRPCars", "cfg/cfg_garages")
m=m.garage_types
...
RageUI.CreateWhile(1.0, RMenu:Get("PlateShop", "main"), nil, function()
    ...
    RageUI.IsVisible(RMenu:Get("PlateShop", "sub"), true, false, true, function()
        RageUI.Button("Change Number Plate", "~g~Changing plate of "..selectedCarName, {RightLabel = "~g~£50,000"}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("SincoRP:ChangeNumberPlate", selectedCar)
            end
        end)
    ...
    while true do
        ...
        if isInArea(location, 1.4) and isInMenu == false then 
            ...
            if IsControlJustPressed(0, 51) then 
                TriggerServerEvent('SincoRP:getCars')
...
RegisterNetEvent("SincoRP:RecieveNumberPlate")
AddEventHandler("SincoRP:RecieveNumberPlate", function(numplate)
...
RegisterNetEvent("SincoRP:carsTable")
AddEventHandler("SincoRP:carsTable",function(cars)
    carstable = cars
end)
