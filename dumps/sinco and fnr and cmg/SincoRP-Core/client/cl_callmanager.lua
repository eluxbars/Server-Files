...
RMenu.Add('callmanager', 'main', RageUI.CreateMenu("", '~r~SincoRP Call Manager', 1300, 50, "banners", "callmanager"))
                    ...
                    if k == ticketID then
                        if v[2] == GetPlayerServerId(PlayerId()) then
                            ...
                            CallManagerServer.GetUpdatedCoords({v[2]}, function(targetCoords)
                                ...
                                TriggerServerEvent("SincoRP:GiveTicketMoney", v[1], v[2], v[3], true)
                            end)  
                            CallManagerServer.RemoveTicket({k, "admin"})
                            name = v[1]
                            isInTicket = true
                            TriggerServerEvent('SincoRP:getTempFromPerm',v[2])
                        end
                    end
                end
            end
...
AddEventHandler("staffon", function(isInTicket)
    ...
    TriggerEvent("SincoRP:vehicleMenu", true, isInTicket)
    if GetEntityHealth(GetPlayerPed(-1)) <= 103 then
    TriggerEvent('SincoRP:FixClient')
    end
end)
...
AddEventHandler("staffoff", function()
    isInTicket = false
    TriggerEvent("SincoRP:vehicleMenu", false, isInTicket) 
end)


RegisterNetEvent('SincoRP:AdminTicketCooldown')
AddEventHandler('SincoRP:AdminTicketCooldown', function(source, Reason)
    if AdminTicketCooldown == false then
        TriggerServerEvent('SincoRP:sendAdminTicket', source, Reason)
...
RegisterNetEvent('SincoRP:PDCallCooldown')
AddEventHandler('SincoRP:PDCallCooldown', function(source, Reason)
    if PDCallCooldown == false then
        TriggerServerEvent('SincoRP:sendPDCall', source, Reason)
        PDCallCooldown = true
        Wait(60000*2)
...
RegisterNetEvent('SincoRP:NHSCallCooldown')
AddEventHandler('SincoRP:NHSCallCooldown', function(source, Reason)
    if NHSCallCooldown == false then
        TriggerServerEvent('SincoRP:sendNHSCall', source, Reason)
...
RegisterNetEvent('SincoRP:PLAYTICKETRECIEVED')
AddEventHandler('SincoRP:PLAYTICKETRECIEVED', function(source)
    PlaySoundFrontend(-1, "Apt_Style_Purchase", "DLC_APT_Apartment_SoundSet", 0)
end)

RegisterNetEvent('SincoRP:sendPermID')
AddEventHandler('SincoRP:sendPermID', function(permid)
    permid = permid
    while isInTicket do
        inRedzone = false
        Wait(0)
