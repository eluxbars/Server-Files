...
RMenu.Add('PDTP', 'main', RageUI.CreateMenu("Teleport Menu", "~r~Teleport Menu", 1350, 50))
RMenu.Add('PDTOMPD', 'main', RageUI.CreateMenu("Teleport Menu", "~r~Teleport Menu", 1350, 50))
...
RageUI.CreateWhile(1.0, RMenu:Get('PDTP', 'main'), nil, function()
    ...
    for k,v in pairs(vk.TP) do
       if hadPDPERMS then
       RageUI.Button("Teleport to Traning Grounds" , nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
            if Selected then
...
RageUI.CreateWhile(1.0, RMenu:Get('PDTOMPD', 'main'), nil, function()
    ...
    for k,v in pairs(vk.TP) do
       if hadPDPERMS then
       RageUI.Button("Teleport to Mission Row" , nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
            if Selected then
        ...
        for k, v in pairs(vk.TP) do 
            ...
            if isInArea(v1, 1.4) then 
                ...
                --if IsControlJustPressed(0, 51) then 
                    TriggerServerEvent('SincoRP:PDVK')
