...
RegisterCommand("farids", function()
    if faridsenabled then
        TriggerServerEvent("SincoRP:IDsAboveHead", true)
    else
        TriggerServerEvent("SincoRP:IDsAboveHead", false)
    end
end)
...
RegisterCommand("hidechat", function()
    TriggerEvent('SincoRP:toggleChat', true)
    notify('~g~Chat Hidden')
end)

RegisterCommand("showchat", function()
    TriggerEvent('SincoRP:toggleChat', false)
    notify('~g~Chat Shown')
end)

RegisterNetEvent("SincoRP:ChangeIDs")
AddEventHandler("SincoRP:ChangeIDs", function(status)
    if status then
        disPlayerNames = 10
