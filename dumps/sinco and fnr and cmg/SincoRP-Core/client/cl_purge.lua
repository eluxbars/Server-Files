...
RegisterCommand("purge", function(source,args,rawCommand)
    purge = not purge
    print(purge)
    TriggerServerEvent('SincoRP:activatePurgeSV', purge)
end)

RegisterCommand("testgun", function(source,args,rawCommand)
    if purge then
        TriggerServerEvent('SincoRP:purgeRespawn')
    end
end)
...
RegisterNetEvent("SincoRP:activatePurge")
AddEventHandler("SincoRP:activatePurge", function()
