...
RegisterNetEvent('SincoRP:pdRadioAnim')
AddEventHandler('SincoRP:pdRadioAnim', function(status, b)
    if status then
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, 19) then
            TriggerServerEvent('SincoRP:pdRadioPerms', false, b)
        else
            if IsControlJustPressed(0, 19) then
                TriggerServerEvent('SincoRP:pdRadioPerms', true, b)
            end
        end
    end
end)
