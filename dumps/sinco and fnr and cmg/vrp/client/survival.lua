  ...
  while true do 
    if IsDisabledControlJustPressed(0,38) then
      if playerCanRespawn and in_coma and secondsTilBleedout < 2 then
        TriggerServerEvent("SincoRP:PoliceCheck")
        TriggerServerEvent("SincoRP:RebelCheck")
        TriggerServerEvent("SincoRP:VIPCheck")
        TriggerServerEvent("SincoRP:getHouses")
        tvRP.respawnPlayer()
        DoScreenFadeIn(900)
        TriggerEvent("SincoRP:StartRespawnCam")
        TriggerEvent("SincoRP:OpenRespawnMenu")
      end
      Wait(1000)
    end
    Wait(0)
...
Citizen.CreateThread(function() -- coma thread
    ...
    while true do
        ...
        if IsEntityDead(GetPlayerPed(-1)) and not in_coma then --Wait for death check
            ...
            vRPserver.MoneyDrop()
            TriggerEvent('SincoRP:5Seconds')
            tvRP.ejectVehicle()
            in_coma = true
            TriggerEvent("vrp:PlaySound", tvRP.getDeathSound())
...
RegisterNetEvent("SincoRP:5Seconds")
AddEventHandler("SincoRP:5Seconds", function()
    DeathAnim = 5
    while DeathAnim <= 5 and DeathAnim >= 0 do
        Wait(1000)
        DeathAnim = DeathAnim - 1
...
RegisterNetEvent("SincoRP:FixClient")
AddEventHandler("SincoRP:FixClient", function()
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsEntityDead(PlayerPedId()) then
            ...
            if GetEntityHealth(GetPlayerPed(-1)) <= 100 and not in_coma then
