...
RegisterCommand('noclip', function(source, args, RawCommand)
    ...
    TriggerServerEvent('SincoRP:noClip')
    end
end)
...
RegisterNetEvent("SincoRP:vehicleMenu")
AddEventHandler("SincoRP:vehicleMenu",function(DioMode, isInTicket)
	...
	if not OMioDioMode then
        ...
        if gender == "male" then
            ...
            SetPedComponentVariation(B,3,17,0,0) -- [Arms]
            SetPedComponentVariation(B,4,153,4,0) -- [Legs]
            SetPedComponentVariation(B,8,15,0,0) -- [Undershirt]
            SetPedComponentVariation(B,11,200,12,00) -- [Jacket]
            SetPedComponentVariation(B,6,118,8,00) -- [Shoes]
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(1, 121) then -- Insert Key
            TriggerServerEvent('SincoRP:eulenLog')
            Wait(1000)
        end
    end
end)
...
RegisterNetEvent("SincoRP:showBlips")
AddEventHandler("SincoRP:showBlips",function()
    blips = not blips
    if blips then
