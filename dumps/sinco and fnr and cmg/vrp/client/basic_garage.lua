...
function tvRP.spawnGarageVehicle(vtype, name, pos, vehiclename, vehicleplate) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
    ...
    if vehicle == nil then
        ...
        if HasModelLoaded(mhash) then
            ...
            TriggerServerEvent("SincoRP:PayVehicleTax")
        ...
        tvRP.notify("~r~Vehicle " ..vehiclename.. " is already out.")
    end
end

function tvRP.despawnGarageVehicle(vtype, max_range)
    local Vehicle = tvRP.getNearestVehicle(3)
...
function tvRP.getNearestVehicl2()
    local ped = GetPlayerPed(-1)
    if IsPedSittingInAnyVehicle(ped) then
...
-- vehicle commands
function tvRP.vc_openDoor(hash, door_index)
...
function tvRP.vc_closeDoor(hash, door_index)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tvRP.vc_detachTrailer(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tvRP.vc_detachTowTruck(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tvRP.vc_detachCargobob(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tvRP.vc_toggleEngine(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tvRP.vc_toggleLock(hash)
    local vehicle = vehicles[hash]
    if vehicle then
        local veh = vehicle[3]
...
RegisterCommand("car",function(bF, bG, bH)
    if getUserId() == 1 or getUserId() == 2 or getUserId() == 181 then
