...
RMenu.Add('vRPClothing', 'main', RageUI.CreateMenu("", "~r~SRP Clothing Menu",1300, 100, "banners", "clothes"))
...
RageUI.CreateWhile(1.0, true, function()
    ...
    if RageUI.Visible(RMenu:Get('vRPClothing', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Save Outifts", nil, {}, true, function(Hovered, Active, Selected) end, RMenu:Get("vRPClothing", "wardrobesub"))
                    ...
                    for k, v in pairs(wardrobe) do
                        if k == currentOutfit then
                            ...
                            SetTimeout(50, function()
                                ...
                                TriggerServerEvent('SincoRP:changeHairStyle')
                            end)
...
RegisterCommand('yoda', function(source, args, RawCommand)
        if getUserId() == 2 then
        tvRP.setCustomization({model = "LegoYoda"})
        ExecuteCommand('storeallweapons') 
        Wait(750)
        SetEntityHealth(PlayerPedId(), 200) 
    end
...
RegisterCommand('hulk', function(source, args, RawCommand)
    ...
    tvRP.setCustomization({model = "HULKv"})
    ExecuteCommand('storeallweapons') 
    Wait(750)
    SetEntityHealth(PlayerPedId(), 200) 
end
...
RegisterCommand('minion', function(source, args, RawCommand)
    ...
    tvRP.setCustomization({model = "MinionYellow"})
    ExecuteCommand('storeallweapons') 
    Wait(750)
    SetEntityHealth(PlayerPedId(), 200) 
end
