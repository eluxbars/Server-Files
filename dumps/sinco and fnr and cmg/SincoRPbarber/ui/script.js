$(document).ready(function(){
  ...
  $('input').change(function(){
    $.post('http://SincoRPbarber/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    if(e.which == 113){ // A pressed
      $.post('http://SincoRPbarber/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('http://SincoRPbarber/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 119 ){ // W pressed
      $.post('http://SincoRPbarber/zoomin', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 115){ // S pressed
      $.post('http://SincoRPbarber/zoomout', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('#tabs label').on('click', function(e){
    //e.preventDefault();
    $.post('http://SincoRPbarber/zoom', JSON.stringify({
  ...
  $(document).keypress(function(e){
    ...
    xTriggered++;
    if(e.which == 13){
      $.post('http://SincoRPbarber/test', JSON.stringify({
        value: xTriggered
      }));
    }
  });
...
function leftWristComponent() {
    $.post('http://SincoRPbarber/updateLeftWristComponent', JSON.stringify({
...
function leftWristTexture() {
    ...
    $.post('http://SincoRPbarber/updateLeftWristTexture', JSON.stringify({
        componentID: $('.montre .active').attr('data'),
...
function rightWristComponent() {
    $.post('http://SincoRPbarber/updateRightWristComponent', JSON.stringify({
...
function rightWristTexture() {
    ...
    $.post('http://SincoRPbarber/updateRightWristTexture', JSON.stringify({
        componentID: $('.montre-right .active').attr('data'),
