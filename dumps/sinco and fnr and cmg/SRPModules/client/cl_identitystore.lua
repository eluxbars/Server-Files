local cfg = module("SRPModules", "cfg/cfg_identitystore")
local inMenu = false

RMenu.Add("SRP:Identity", "main", RageUI.CreateMenu("Identity Shop", "~b~Identity Shop", GetRageUIMenuWidth(),GetRageUIMenuHeight()))

RageUI.CreateWhile(1.0, RMenu:Get("SRP:Identity", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("SRP:Identity", "main"), true, false, true, function()

        RageUI.ButtonWithStyle("Change Identity", nil, {RightLabel = "£"..cfg.price}, true, function(Hovered, Active, Selected)
            if Selected then
                            ...
                            if secondName ~= "" then  
                                ...
                                if (GetOnscreenKeyboardResult()) then
                                    ...
                                    if tonumber(age) and tonumber(age) < 100 then 
                                        TriggerServerEvent("SRP:ChangeIdentity", firstName, secondName, tonumber(age))
                                    else
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if isInArea(cfg.location, 1.0) and inMenu == false then
            ...
            RageUI.Visible(RMenu:Get("SRP:Identity", "main"), true)
            inMenu = true
            end
        end
        ...
        if isInArea(cfg.location, 1.0) == false and inMenu then
            inMenu = false
            RageUI.Visible(RMenu:Get("SRP:Identity", "main"), false)
        end
    end
end)
