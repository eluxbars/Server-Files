...
RegisterNetEvent('SRP:pdRadioAnim')
AddEventHandler('SRP:pdRadioAnim', function(status, b)
    if status then
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, 19) then
            TriggerServerEvent('SRP:pdRadioPerms', false, b)
        else
            if IsControlJustPressed(0, 19) then
                TriggerServerEvent('SRP:pdRadioPerms', true, b)
            end
        end
    end
end)
