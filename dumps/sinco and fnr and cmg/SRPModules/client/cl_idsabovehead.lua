...
Citizen.CreateThread(function()
    ...
    while true do
        if faridsenabled then
            drawNativeText("~r~You currently have Far ID's on.", 255, 0, 0, 255, true)
        end
        Citizen.Wait(0)
    end
end)
...
RegisterCommand("farids", function()
    if faridsenabled then
        TriggerServerEvent("SRP:IDsAboveHead", true)
    else
        TriggerServerEvent("SRP:IDsAboveHead", false)
    end
end)
...
RegisterCommand("hidechat", function()
    TriggerEvent('SRP:toggleChat', true)
    notify('~g~Chat Hidden')
end)

RegisterCommand("showchat", function()
    TriggerEvent('SRP:toggleChat', false)
    notify('~g~Chat Shown')
end)

RegisterNetEvent("SRP:ChangeIDs")
AddEventHandler("SRP:ChangeIDs", function(status)
    if status then
        disPlayerNames = 10
