...
local m = module("SRPVehicles", "cfg/cfg_garages")
m=m.garage_types

RMenu.Add('PlateShop', 'main', RageUI.CreateMenu("Number Plate", "~b~Number Plate", GetRageUIMenuWidth(),GetRageUIMenuHeight()))
RMenu.Add("PlateShop", "sub", RageUI.CreateSubMenu(RMenu:Get("PlateShop", "main"), "Number Plate", "~b~Number Plate", GetRageUIMenuWidth(),GetRageUIMenuHeight()))
...
RageUI.CreateWhile(1.0, RMenu:Get("PlateShop", "main"), nil, function()
    ...
    RageUI.IsVisible(RMenu:Get("PlateShop", "sub"), true, false, true, function()
        RageUI.ButtonWithStyle("Change Number Plate", "~g~Changing plate of "..selectedCarName, {RightLabel = "~g~£50,000"}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("SRP:ChangeNumberPlate", selectedCar)
            end
        end)
    ...
    while true do
        ...
        if isInArea(location, 1.4) and isInMenu == false then 
            ...
            if IsControlJustPressed(0, 51) then 
                TriggerServerEvent('SRP:getCars')
...
RegisterNetEvent("SRP:RecieveNumberPlate")
AddEventHandler("SRP:RecieveNumberPlate", function(numplate)
...
RegisterNetEvent("SRP:carsTable")
AddEventHandler("SRP:carsTable",function(cars)
    carstable = cars
end)
