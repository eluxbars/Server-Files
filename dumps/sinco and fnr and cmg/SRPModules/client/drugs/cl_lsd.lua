        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              SRPcfgdrugsServer.CanProcessLSD({}, function(canProcess, ketAmount)
...
RMenu.Add('LSDMenu', 'LSD Seller', RageUI.CreateMenu("", "~p~LSD Seller",1300, 50, "banners", "LSD"))
...
RageUI.CreateWhile(1.0, RMenu:Get('LSDMenu', 'LSD Seller'):SetPosition(1300, 50), nil, function()
  RageUI.IsVisible(RMenu:Get('LSDMenu', 'LSD Seller'), true, false, true, function()
    ...
    RageUI.ButtonWithStyle("Sell LSD" , nil, {RightLabel = "~g~£50,000"}, true, function(Hovered, Active, Selected)
        if Selected then
            sellingLSDUnits()
        end
    end)
...
function sellingLSDUnits()
    SRPcfgdrugsServer.SellLSD({tonumber(1)})
    return false
end
