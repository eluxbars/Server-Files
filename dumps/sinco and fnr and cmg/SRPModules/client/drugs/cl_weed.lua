        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              SRPcfgdrugsServer.CanProcessWeed({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then

          


                  SRPcfgdrugsServer.ProcessWeed()
...
RageUI.CreateWhile(1.0, RMenu:Get('WeedMenu', 'Weed Seller'):SetPosition(1300, 50), nil, function()
  RageUI.IsVisible(RMenu:Get('WeedMenu', 'Weed Seller'), true, false, true, function()
    ...
    RageUI.ButtonWithStyle("Sell Weed" , nil, {RightLabel = "~g~£2,500"}, true, function(Hovered, Active, Selected)
        if Selected then
            sellingWeedUnits()
        end
    end)
...
function sellingWeedUnits()
    SRPcfgdrugsServer.SellWeed({tonumber(1)})
    return false
end
