...
RegisterCommand("purge", function(source,args,rawCommand)
    purge = not purge
    print(purge)
    TriggerServerEvent('SRP:activatePurgeSV', purge)
end)

RegisterCommand("testgun", function(source,args,rawCommand)
    if purge then
        TriggerServerEvent('SRP:purgeRespawn')
    end
end)
...
RegisterNetEvent("SRP:activatePurge")
AddEventHandler("SRP:activatePurge", function()
