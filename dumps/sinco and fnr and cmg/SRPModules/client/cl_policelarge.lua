...
RMenu.Add('policelarge', 'main', RageUI.CreateMenu("", "~b~ Police Armoury", 1300, 50, "banners", "police"))
RMenu.Add("policelarge", "sub", RageUI.CreateSubMenu(RMenu:Get("policelarge", "main"), "", "~b~ Police Armoury", 1300, 50, "banners", "police"))
RMenu.Add("policelarge", "whitelistedguns", RageUI.CreateSubMenu(RMenu:Get("policelarge", "main"), "", "~b~ Police Armoury", 1300, 50, "banners", "police"))
RageUI.CreateWhile(1.0, RMenu:Get('policelarge', 'main'), nil, function()
    RageUI.IsVisible(RMenu:Get('policelarge', 'main'), true, false, true, function()
        for i , p in pairs(policelarge.guns) do 
            RageUI.ButtonWithStyle(p.name , nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if Active then
                    currentGunHash = nil
                end
                if Selected then
        ...
        for i , w in pairs(returnedPDGuns2) do 
            RageUI.ButtonWithStyle(w.name , nil, {RightLabel = '→→→'}, true, function(Hovered, Active, Selected)
                if Active then
                    currentGunHash1 = nil
                end
                if Selected then
    ...
    RageUI.IsVisible(RMenu:Get("policelarge", "sub"), true, false, true, function()
        ...
        RageUI.ButtonWithStyle("Buy Max Ammo", "Purchase Max Ammo for "..currentGunName, {  RightLabel = "~g~£"..tostring(getMoneyStringFormatted(math.floor(currentGunPrice / 2))) }, true, function(Hovered, Active, Selected)
            --RageUI.ButtonWithStyle("Buy Max Ammo", "Purchase Max Ammo for "..currentGunName, { RightLabel = "~g~£"..tostring(getMoneyStringFormatted(math.floor(currentGunPrice / 2))) }, true, function(Hovered, Active, Selected)v
            if Selected then
    ...
    RageUI.IsVisible(RMenu:Get("policelarge", "whitelistedguns"), true, false, true, function()
        RageUI.ButtonWithStyle("Purchase Weapon Body" , "Purchase "..currentGunName1.." and Max Ammo", {  RightLabel = "~g~£"..tostring(getMoneyStringFormatted(currentGunPrice1)) }, true, function(Hovered, Active, Selected)
            if Selected then
                local Ped = PlayerPedId()
