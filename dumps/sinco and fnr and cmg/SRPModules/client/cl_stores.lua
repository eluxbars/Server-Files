local cfg = module("SRPModules", "cfg/cfg_stores")
local inMenu = false
...
RMenu.Add("SRP:Shops", "main", RageUI.CreateMenu("Shop", "~b~Shop", GetRageUIMenuWidth(),GetRageUIMenuHeight()))
RMenu.Add("SRP:Shops", "sub", RageUI.CreateSubMenu(RMenu:Get("SRP:Shops", "main"), "Shop", "~b~Confirm Purchase", GetRageUIMenuWidth(),GetRageUIMenuHeight()))

RageUI.CreateWhile(1.0, RMenu:Get("SRP:Shops", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("SRP:Shops", "main"), true, false, true, function()
        for k, v in pairs(cfg.items) do
            if v.type == 'store' then
                RageUI.ButtonWithStyle(v.name, "£"..getMoneyStringFormatted(v.price), {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        currentItemID = v.itemID
                        currentItemPrice = v.price
                        currentItemName = v.name
                    end
                end, RMenu:Get("SRP:Shops", "sub", true))
            end
        end
    ...
    RageUI.IsVisible(RMenu:Get("SRP:Shops", "sub"), true, false, true, function()
        RageUI.List("Amount", maxAmount, amount, nil, {}, true, function(Hovered, Active, Selected, Index)
            amount = Index
        end)
        RageUI.ButtonWithStyle("~g~Confirm Purchase",nil,  {RightLabel = "£"..getMoneyStringFormatted(currentItemPrice * amount)}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("SRP:BuyShopItem", currentItemID, amount)
            end
        end, RMenu:Get("SRP:Shops", "main", true))
        ...
        for k, v in pairs(cfg.shops) do
            ...
            if isInArea(v, 1.0) and inMenu == false then
                ...
                if IsControlJustPressed(0, 51) then 
                    inMenu = true
                    currentShop = k
                    RageUI.Visible(RMenu:Get("SRP:Shops", "main"), true)
                end
            end
