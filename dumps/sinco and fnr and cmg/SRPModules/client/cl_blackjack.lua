...
RMenu.Add('SRPblackjack', 'instructions', RageUI.CreateMenu("", "test",GetRageUIMenuWidth(),GetRageUIMenuHeight(),"casinoui_cards_blackjack", "casinoui_cards_blackjack"))
RMenu:Get('SRPblackjack', 'instructions'):SetSubtitle("~b~BLACKJACK")
RMenu.Add('SRPblackjack_high', 'instructions', RageUI.CreateMenu("", "test",GetRageUIMenuWidth(),GetRageUIMenuHeight(),"casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high"))
RMenu:Get('SRPblackjack_high', 'instructions'):SetSubtitle("~b~BLACKJACK")


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('SRPblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            ...
            end, RMenu:Get('SRPblackjack', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('SRPblackjack_high', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "The aim of Blackjack is to beat the Dealer's hand without going over 21. This game uses four 52           card decks, which are shuffled at the start of every hand.                                                                                    The dealer will stand on soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
            ...
            end, RMenu:Get('SRPblackjack_high', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
function showHowToBlackjack(flag)
    if closestChair < 8 then 
        RageUI.Visible(RMenu:Get('SRPblackjack', 'instructions'), flag)
    else 
        RageUI.Visible(RMenu:Get('SRPblackjack_high', 'instructions'), flag)
    end
end
