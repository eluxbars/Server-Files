RMenu.Add('SRPtruckjob', 'main', RageUI.CreateMenu("", "~g~Truck Job", nil, nil, "banners", ""))
RMenu:Get('SRPtruckjob', 'main'):SetPosition(1350, 10)
...
local truckJobPay = 0
RageUI.CreateWhile(1.0, RMenu:Get('SRPtruckjob', 'main'), nil, function()

    RageUI.IsVisible(RMenu:Get('SRPtruckjob', 'main'), true, false, true, function()
            ...
            RageUI.ButtonWithStyle(v.name , "~g~On completion you will receive £" ..Comma(v.pay), { RightLabel = '→→→'}, true, function(Hovered, Active, Selected)
                if Selected then
                    if not jobStarted then
                        ...
                        TriggerServerEvent('SRP:startTruckJob', v.name)
                    else
        ...
        if jobStarted then
            RageUI.ButtonWithStyle("End Job" , "~r~Stop your current job", { RightLabel = '→→→'}, true, function(Hovered, Active, Selected)
                if Selected then
                    if jobStarted then
                        jobStarted = false
                        truckJobCoords = nil
                        ...
                        TriggerServerEvent('SRP:finishTruckJob')
                        truckJobPay = 0
                    else
        ...
        if isInArea(vector3(truckjob.startLocation), 2.5) then 
            ...
            if IsControlJustPressed(0, 51) then 
                if not IsPedSittingInAnyVehicle(PlayerPedId()) then
                    RageUI.CloseAll()
                    RageUI.Visible(RMenu:Get("SRPtruckjob", "main"), true)
                else
                    ...
                    if jobStarted then
                        if (IsPedSittingInAnyVehicle(PlayerPedId())) then
                            if IsVehicleModel(GetVehiclePedIsIn(PlayerPedId(), true), GetHashKey(truckjob.vehicle)) then
                                ...
                                TriggerServerEvent('SRP:finishTruckJob', currentTruckJob)
                                jobStarted = false
                                truckJobCoords = nil
...
function SpawnTruck()
	...
	SetVehicleNumberPlateText(spawned_car, "SRP")
