  ...
  Citizen.Wait(0)
      for k, v in pairs(bmxSpawns) do
          ...
          if isInArea(v, 1.4) then 
              TriggerEvent('SRP:bmxSpawner')
          end
      end
  end
end)

RegisterNetEvent('SRP:bmxSpawner')
AddEventHandler('SRP:bmxSpawner', function()
    alert('~g~Press ~INPUT_VEH_HORN~ to spawn ~b~Taxi')
    if IsControlJustPressed(0, 51) then
      ...
      if bmxCooldown == true then
        notify('~r~You have to wait 1 minute before you can spawn another Taxi.')
      end
    end
end)
