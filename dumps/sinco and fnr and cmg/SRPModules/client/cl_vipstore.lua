...
RMenu.Add("VIPMenu", "sub", RageUI.CreateSubMenu(RMenu:Get("VIPMenu", "main"), "", "~y~VIP Gun Store", 1300, 50, "banners", "vip"))
RMenu.Add("VIPMenu", "whitelistedguns", RageUI.CreateSubMenu(RMenu:Get("VIPMenu", "main"), "", "~y~VIP Gun Store", 1300, 50, "banners", "vip"))
...
RageUI.CreateWhile(1.0, RMenu:Get("VIPMenu", "main"), nil, function()
    ...
    RageUI.IsVisible(RMenu:Get("VIPMenu", "main"), true, false, true, function()
        ...
        for i , o in pairs(returnedGuns2) do 
            RageUI.ButtonWithStyle(o.name , "~g~£" .. o.price, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if Active then
                    currentGunHash1 = nil
                    hoveredArmour = false
        ...
        RageUI.ButtonWithStyle("Level 4 Armour [Max]" , "~g~£"..getMoneyStringFormatted(VIP.maxArmourPrice), {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
            if Active then
                hoveredArmour = true
            end
            if Selected then
    ...
    while true do
        ...
        for k,v in pairs(VIP.gunshops) do
            if currentGunHash ~= nil or currentGunHash1 ~= nil or hoveredArmour then
                ...
                O=CreateObject(N,v.x,v.y,v.z+0.3,false,false,false)
