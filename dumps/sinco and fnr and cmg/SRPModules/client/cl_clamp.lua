...
SRPClampClientT = {}
Tunnel.bindInterface("SRP_Clamp",SRPClampClientT)
Proxy.addInterface("SRP_Clamp",SRPClampClientT)
SRPClampServer = Tunnel.getInterface("SRP_Clamp","SRP_Clamp")
vRP = Proxy.getInterface("vRP")

Clamps = {}
DisabledVehs = {}

function SRPClampClientT.ClampVehicle()
  ...
  if DoesEntityExist(veh) then
    ...
    DecorSetInt(clamp,"SRPVehicles",955)
    ...
    SRPClampServer.ChangeVehState({VehToNet(veh), true})
...
function SRPClampClientT.ChangeVehState(veh, disable)
  if disable then
...
RegisterCommand("removeclamps", function(source, args, rawCommand)
  for k, v in pairs(Clamps) do
    if v ~= nil then
      DeleteEntity(v[1])
      SRPClampServer.ChangeVehState({VehToNet(v[2]), false})
    end
  end
  Clamps = {}
...
RegisterNetEvent("SRP:UnClampVehicles")
AddEventHandler("SRP:UnClampVehicles", function()
  for k, v in pairs(Clamps) do
    if v ~= nil then
      DeleteEntity(v[1])
      SRPClampServer.ChangeVehState({VehToNet(v[2]), false})
...
RegisterCommand("removeclamp", function(source, args, rawCommand)
  ...
  if Clamps[ID] == nil then
    ...
    SRPClampServer.ChangeVehState({VehToNet(Clamps[ID][2]), false})
    Clamps[ID] = nil
