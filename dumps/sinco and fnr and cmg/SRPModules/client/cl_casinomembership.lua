...
RMenu.Add("SRP:CasinoLicenseStore", "main", RageUI.CreateMenu("", "~b~Casino License Store", GetRageUIMenuWidth(),GetRageUIMenuHeight(),"banners","licenses"))
RMenu.Add("SRP:CasinoLicenseStore", "sub", RageUI.CreateSubMenu(RMenu:Get("SRP:CasinoLicenseStore", "main"), "", "~b~License Store", GetRageUIMenuWidth(),GetRageUIMenuHeight()))
RMenu.Add("SRP:CasinoLicenseStore", "remove", RageUI.CreateSubMenu(RMenu:Get("SRP:CasinoLicenseStore", "main"), "", "~b~Confirm Remove License", GetRageUIMenuWidth(),GetRageUIMenuHeight()))
RMenu.Add("SRP:CasinoLicenseStore", "confirm", RageUI.CreateSubMenu(RMenu:Get("SRP:CasinoLicenseStore", "sub"), "", "~b~Confirm License Purchase", GetRageUIMenuWidth(),GetRageUIMenuHeight()))

RageUI.CreateWhile(1.0, RMenu:Get("SRP:CasinoLicenseStore", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("SRP:CasinoLicenseStore", "main"), true, false, true, function()

        RageUI.ButtonWithStyle("Casino Licenses", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
            if Selected then
                currentType = "casino"
            end
        end, RMenu:Get("SRP:CasinoLicenseStore", "sub", true))

    end, function() 
    end)

    RageUI.IsVisible(RMenu:Get("SRP:CasinoLicenseStore", "sub"), true, false, true, function()
            ...
            if v.type == currentType then
                RageUI.ButtonWithStyle(v.name, nil, {RightLabel = "~g~£"..getMoneyStringFormatted(v.price)}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        RMenu:Get("SRP:CasinoLicenseStore", "confirm"):SetSubtitle("~b~Are you sure you want to buy "..v.name.."?")
                    end
                end, RMenu:Get("SRP:CasinoLicenseStore", "confirm", true))
            end
        end
        ...
        end, RMenu:Get("SRP:CasinoLicenseStore", "remove", true))

    end, function() 
    end)

    RageUI.IsVisible(RMenu:Get("SRP:CasinoLicenseStore", "confirm"), true, false, true, function()
        ...
        end, RMenu:Get("SRP:CasinoLicenseStore", "main", true))
    ...
    RageUI.IsVisible(RMenu:Get("SRP:CasinoLicenseStore", "remove"), true, false, true, function()
        ...
        RageUI.ButtonWithStyle("~g~Confirm Removal", nil, {RightLabel = "~g~£0"}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("SRP:RemoveLicense", currentLicense)
            end
        end, RMenu:Get("SRP:CasinoLicenseStore", "main", true))
    ...
    while true do
        ...
        if isInArea(cfg.casinolicenseShopLocation, 1.0) and inMenu == false then
            ...
            if IsControlJustPressed(0, 51) then 
                inMenu = true
                RageUI.Visible(RMenu:Get("SRP:CasinoLicenseStore", "main"), true)
            end
        end
