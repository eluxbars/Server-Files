...
cfgweapons = module("FNRWeap", "cfg/cfg_weaponNames")
local user_id = 0
tvRP = {}
allowedWeapons = {}
...
function tvRP.setBasePlayers(players)
  baseplayers = players
end

function tvRP.addBasePlayer(player, id)
  baseplayers[player] = id
end

function tvRP.removeBasePlayer(player)
  --baseplayers[player] = nil
end

function tvRP.setUserID(a)
  user_id = a
end
function tvRP.getUserId(Z)
  if Z then
...
-- looping: if true, will FNRly loop the first element of the sequence until stopAnim is called
...
Citizen.CreateThread(
    function()
        while true do
            local ay = {}
            ay.playerPed = tvRP.getPlayerPed()
            ay.playerCoords = tvRP.getPlayerCoords()
            ay.playerId = tvRP.getPlayerId()
            ay.vehicle = tvRP.getPlayerVehicle()
...
function tvRP.createThreadOnTick(aA)
  ax[#ax + 1] = aA
end
local ai = 0
local R = 0
...
AddEventHandler("playerSpawned",function()
  TriggerEvent("FNR:UnFreezeRespawn")
end)
...
local BucketValue = 0
function tvRP.getPlayerBucket()
    return BucketValue
end
RegisterNetEvent("FNR:SetPlayerBucket",function(BucketNumber)
