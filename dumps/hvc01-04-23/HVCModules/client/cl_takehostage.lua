...
function callTakeHostage()
	...
	if IsPedSittingInAnyVehicle(PlayerPedId()) then
		drawNativeNotification("~r~You Cannot Be In A Vehicle Wile Trying To Take Someone Hostage.")
		return
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				local Anim = IsEntityPlayingAnim(GetPlayerPed(closestPlayer), "missminuteman_1ig_2", "handsup_enter", 3)
                if Anim then
