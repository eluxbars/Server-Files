...
local function DrawButton(b)
	...
	if IsDisabledControlJustPressed(1, 24,true) then
		local x,y = GetCursor()
		local Distance = Distance(b.x+0.005, b.y+0.025, x, y)
		if Distance < 0.025 then return true end
	elseif IsDisabledControlJustPressed(1, 25,true) and Config.Debug then
		local x,y = GetCursor()
		local Distance = Distance(b.x+0.005, b.y+0.025, x, y)
...
Citizen.CreateThread(function()
	...
	while true do Wait(0)
		if IsControlJustPressed(1, Config.GUI.Key,true) then
			local Ped = PlayerPedId() 
			if Check(Ped) then SoundPlay("Open") SetCursorLocation(Config.GUI.Position.x, Config.GUI.Position.y) end
		elseif IsControlPressed(1, Config.GUI.Key,true) then
			local Ped = PlayerPedId() 
			if Check(Ped) then DrawGUI() end
		elseif IsControlJustReleased(1, Config.GUI.Key,true) then
