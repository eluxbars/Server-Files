...
RegisterNetEvent('HVC:isPoliceVar')
AddEventHandler('HVC:isPoliceVar', function(target)

end)

RegisterNetEvent('HVC:getTackled')
AddEventHandler('HVC:getTackled', function(target)
...
RegisterNetEvent('HVC:playTackle')
AddEventHandler('HVC:playTackle', function()
	isTackling = true
	local playerPed =PlayerPedId()
	...
	while true do
		...
		if IsControlPressed(0, Keys['LEFTSHIFT'], true) and IsControlPressed(0, Keys['G'], true) and not isTackling then
			...
			if distance ~= -1 and distance <= 3.0 and not isTackling and not isGettingTackled and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				TriggerServerEvent('HVC:tryTackle', closestPlayerId)
			end
		end
	end
end)
