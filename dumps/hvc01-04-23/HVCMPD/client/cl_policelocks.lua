...
RegisterNetEvent('HVC:load')
AddEventHandler('HVC:load', function(list)
	doors = list
end)

RegisterNetEvent('HVC:statusSend')
AddEventHandler('HVC:statusSend', function(i, status)
	...
	while true do
		...
		if not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
			if IsControlJustPressed(table.unpack(LockHotkey)) then
				...
				if id ~= 0 then
					TriggerServerEvent("HVC:open", id)
				end
			end
		end
	end
