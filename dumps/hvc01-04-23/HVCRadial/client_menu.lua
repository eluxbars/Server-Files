...
function Crosshair(enable)
    TriggerEvent("HVC:PutCrossHairOnScreen", enable)
    Display = enable;
end
            ...
            if playerIsAlive() and playerVehicle == 0 then
                if EntityType == 1 and Entity ~= playerPed and IsPedAPlayer(Entity) then
                    ...
                    if IsDisabledControlPressed(0, keybindControls[keyBind]) and GetLastInputMethod(2) and not showMenu then
                        TriggerServerEvent("HVC:PoliceCheck")
                        TriggerServerEvent("HVC:NHSCheck")
                        showMenu = true
                        local enabledMenus = {}
...
AddEventHandler("onClientResourceStop", function(resourceName)
    TriggerServerEvent("HVC:AC", "Resource Stop Client: "..resourceName)
end)

RegisterNetEvent("HVC:CleanCar")
AddEventHandler("HVC:CleanCar",function()
    if GetEntityHealth(PlayerPedId()) >= 103 and not IsEntityDead(PlayerPedId()) then 
        showMenu = false
        SendNUIMessage({
            menu = false
        })
        ...
        SetNuiFocus(false, false)
        TaskStartScenarioInPlace(PlayerPedId(), 'world_human_maid_clean', 0, 1)
        Wait(10000)
        SetVehicleDirtLevel(GetVehiclePedIsIn(PlayerPedId(),true), 0.0)
        SetVehicleUndriveable(GetVehiclePedIsIn(PlayerPedId(),true), false)
        ClearPedSecondaryTask(PlayerPedId())
        ClearPedTasks(PlayerPedId())
    end
end)

--RegisterNetEvent("HVC:RepairVeh")
--AddEventHandler("HVC:RepairVeh",function()
